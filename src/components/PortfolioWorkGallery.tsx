      ...
      <div className="mx-auto max-w-7xl">
            ...
            >
              ...
              <div className="relative grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4">
                {["PROBLEM", "BRAND", "MOBILE", "LAUNCH"].map((label, index) => (
                  <span
                    key={label}
                    className="border border-white/10 bg-[#030014]/70 p-4 font-mono text-[10px] tracking-widest text-white/65 transition-colors group-hover:border-[#DFBA73]/40"
                  >
                    0{index + 1}
                    <span className="mt-8 block text-white">{label}</span>
                  </span>
                ))}
              </div>
            </Link>
        ...
        <ScrollReveal className="mt-8" respectEffectsGuard>
          <aside className="grid gap-6 border border-[#FF3333]/35 bg-[#FF3333]/5 p-7 md:grid-cols-[minmax(0,1fr)_auto] md:items-center md:p-9">
            ...
            <Link
              to="/audit"
              className="inline-flex min-h-11 w-fit items-center gap-2 bg-[#FF3333] px-6 py-3 font-mono text-xs font-bold tracking-widest text-[#030014] transition-colors hover:bg-[#DFBA73]"
            >
