...
const TARGETS: Record<Chapter, Target> = {
  hero: { x: ORE_X, y: ORE_Y, scale: 1, dim: 1, refine: 0, spin: 0.03, key: 2.2, furnace: 4.5, cameraZ: CAMERA_Z },
  work: { x: ORE_X + 0.3, y: ORE_Y, scale: 0.7, dim: 0.2, refine: 0.15, spin: 0.03, key: 0.9, furnace: 3, cameraZ: CAMERA_Z + 1.5 },
  services: { x: ORE_X, y: ORE_Y, scale: 0.9, dim: 0.55, refine: 0.35, spin: 0, key: 2.2, furnace: 2, cameraZ: CAMERA_Z },
  promise: { x: 0.8, y: ORE_Y, scale: 1, dim: 0.9, refine: 0.55, spin: 0, key: 2.2, furnace: 2, cameraZ: CAMERA_Z },
  about: { x: ORE_X, y: ORE_Y - 0.6, scale: 0.85, dim: 0.6, refine: 0.8, spin: 0.015, key: 1.6, furnace: 1, cameraZ: CAMERA_Z },
  finale: { x: ORE_X, y: ORE_Y + 0.2, scale: 1, dim: 1, refine: 1, spin: 0.02, key: 2.6, furnace: 0, cameraZ: CAMERA_Z },
};
  ...
  useEffect(() => {
    ...
    const initialise = async () => {
      ...
      for (let index = 0; index < orePositions.count; index += 1) {
        normal.set(orePositions.getX(index), orePositions.getY(index), orePositions.getZ(index)).normalize();
        ...
        orePositions.setXYZ(index, orePositions.getX(index) + normal.x * displacement, orePositions.getY(index) + normal.y * displacement, orePositions.getZ(index) + normal.z * displacement);
      }
      ...
      const oreMaterial = new THREE.MeshPhysicalMaterial({ color: 0x1a1714, roughness: 0.85, metalness: 0, envMapIntensity: 1.15, clearcoat: 0.08 });
...
diffuseColor.rgb = mix(vec3(0.0103, 0.0086, 0.0069), vec3(0.738, 0.491, 0.171), oreMask);`)
          ...
          .replace("#include <opaque_fragment>", "outgoingLight *= uDim;\n#include <opaque_fragment>");
      };
      ...
      const glowMaterial = new THREE.SpriteMaterial({ map: glowTexture, color: 0xdfba73, transparent: true, opacity: 0.42, blending: THREE.AdditiveBlending, depthWrite: false });
      ...
      const dustUniforms = { uTime: { value: 0 }, uPixelRatio: { value: renderer.getPixelRatio() }, uDim: { value: 1 } };
      ...
      const applyScene = (now: number, delta: number, snap = false) => {
        ...
        for (const key of ["x", "y", "scale", "dim", "refine", "spin", "key", "furnace", "cameraZ"] as const) current[key] = lerp(current[key], target[key], amount);
        ...
        if (activeChapter === "services" && serviceRotation !== null) {
          if (snap) ore.rotation.y = serviceRotation;
            ...
            const difference = Math.atan2(Math.sin(serviceRotation - ore.rotation.y), Math.cos(serviceRotation - ore.rotation.y));
            serviceVelocity = (serviceVelocity + difference * Math.min(delta * 18, 1)) * Math.pow(0.6, delta * 60);
        ...
        oreUniforms.uVeinGlow.value = (0.07 + (Math.sin((elapsed / 6) * Math.PI * 2) * 0.5 + 0.5) * 0.08) * effectiveDim;
      ...
      const chapterElements = [...document.querySelectorAll<HTMLElement>("[data-refinery-chapter]")];
      const detectChapter = () => {
      ...
      const chapterObserver = new IntersectionObserver(detectChapter, { rootMargin: "-50% 0px -50% 0px", threshold: 0 });
    ...
    if (typeof idleWindow.requestIdleCallback === "function") idleHandle = idleWindow.requestIdleCallback(() => void initialise());
  ...
  return <div ref={holder} aria-hidden className="refinery-scene refinery-fixed-layer pointer-events-none fixed inset-0" />;
}
