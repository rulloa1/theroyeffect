    ...
    <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#030014]/85 backdrop-blur-xl">
      <div className="mx-auto grid max-w-[96rem] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 sm:flex sm:justify-between md:px-8">
        ...
        <nav aria-label="Primary" className="hidden shrink-0 items-center gap-6 lg:flex">
          {links.map((link) => (
            <Link key={link.label} to={link.to} className="inline-flex min-h-11 items-center font-mono text-[10px] font-bold tracking-widest text-white/75 transition-colors hover:text-[#DFBA73]">
              {link.label}
            </Link>
          ))}
          <a href="mailto:rory@theroyeffect.com" className="inline-flex min-h-11 items-center font-mono text-[10px] font-bold tracking-widest text-white/75 transition-colors hover:text-[#DFBA73]">CONTACT</a>
        </nav>
      ...
      {menuOpen ? (
        <nav aria-label="Mobile primary" className="border-t border-white/10 bg-[#030014] px-5 py-5 lg:hidden">
          {[...links, { label: "CONTACT", to: "/brief" as const }].map((link) => (
            <Link key={link.label} to={link.to} onClick={() => setMenuOpen(false)} className="flex min-h-12 items-center justify-between border-b border-white/10 font-portfolio text-xl font-semibold text-white last:border-0">
