...
function WorkImage({ entry, index, className }: { entry: ShowcaseWorkEntry; index: number; className?: string }) {
  ...
  return (
    <div className={`relative overflow-hidden bg-[#0a0620] ${className ?? ""}`}>
      {showImage ? (
        <img ref={imgRef} src={entry.image ?? ""} alt={entry.alt} loading="lazy" decoding="async" onError={() => setImageFailed(true)} className="absolute inset-0 h-full w-full object-cover" />
      ) : (
...
export function WorkGrid({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (previewRef.current) previewRef.current.style.transform = `translate3d(${smoothRef.current.x + 20}px, ${smoothRef.current.y - 110}px, 0)`;
  ...
  return (
    <div>
      <div ref={containerRef} onMouseMove={movePreview} className="relative">
        {compact ? (
          <div className="grid gap-4 md:grid-cols-3">{SHOWCASE_WORK.map((entry, index) => <CompactCard key={entry.slug} entry={entry} index={index} />)}</div>
        ) : (
          <div className="border-b border-white/10">
            {SHOWCASE_WORK.map((entry, index) => (
              <Link key={entry.slug} to="/case-study" onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} className="group block border-t border-white/10 bg-white/[0.02] px-4 py-5 transition-colors hover:bg-white/[0.04] md:px-6 md:py-7">
                <WorkImage entry={entry} index={index} className="mb-5 aspect-[4/3] w-full border border-[#DFBA73]/30 md:hidden" />
                <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(14rem,0.75fr)_auto] md:items-center md:gap-8">
                  <div className="flex items-center gap-3"><h3 className="font-display text-3xl uppercase text-white transition-colors group-hover:text-[#DFBA73] md:text-5xl">{entry.title}</h3><ArrowUpRight className="size-5 -translate-x-2 text-white opacity-0 transition-all group-hover:translate-x-0 group-hover:text-[#DFBA73] group-hover:opacity-100" /></div>
                  <p className="font-mono text-base leading-[1.6] text-white/90">{entry.result}</p>
                  <span className="font-mono text-xs tracking-widest text-[#DFBA73] md:max-w-44 md:text-right">{entry.eyebrow}</span>
                </div>
              </Link>
            ))}
          </div>
